...
// Fonction pour calculer Temp_Max_C en Celsius
var tempMaxClimate = ee.ImageCollection(years.map(function(year) {
...
// Visualiser sur la carte : exemple d'août 2023
var exampleMonth = tempMaxClimate.filter(ee.Filter.eq('year', 2023))
                                 .filter(ee.Filter.eq('month', 8)).first();

Map.centerObject(france, 5);

// Ajouter des couches à la carte
Map.addLayer(exampleMonth.select('Temp_Max_C'), 
             {min: -5, max: 30, palette: ['blue', 'yellow', 'red']}, 
             'Temp Max Août 2023 (°C)');

// Exporter Temp_Max_C vers Google Drive
years.getInfo().forEach(function(year) {
  months.getInfo().forEach(function(month) {
    var image = tempMaxClimate.filter(ee.Filter.eq('year', year))
                              .filter(ee.Filter.eq('month', month)).first();
    if (image) {
      Export.image.toDrive({
        image: image,
        description: 'Max_Temp_France_' + year + '_' + month,
        folder: 'TerraClimate_Max',
        fileNamePrefix: 'Max_Temp_France_' + year + '_' + month,
