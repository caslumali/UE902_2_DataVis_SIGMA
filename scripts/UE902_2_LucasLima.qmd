---
title: "Analyse de la variabilité des températures en France"
subtitle: "dans les deux dernières décennies (2004-2023)"
author: "Lucas Lima"
...
execute:
  freeze: auto
description: "Projet de l'UE 902_2 Data Visualisation du M2 SIGMA UT2/ENSAT"
---
...
## Les six visualisations proposées
...
Une fois les données préparées, tous les **géotraitements** ont été effectués et les visualisations ont été intégrées dans une page HTML interactive. Cette dernière a été hébergée sur un **Always Data** pour permettre son partage et sa consultation en ligne.
...
total_layers <- nlyr(raster_temp_mean) # Total de couches (240)
...
-   **Variabilité Saisonnière Marquée :** Chaque courbe représente la distribution des températures pour l’ensemble du territoire sur l’ensemble des 20 années, pour un mois donné (tous les janvier confondus, tous les février confondus, etc.). Les écarts prononcés entre les mois d’hiver et d’été soulignent la forte saisonnalité du climat français.

-   **Transitions Progressives entre les Saisons :** L’évolution de la forme et de la position des courbes de densité d’un mois à l’autre reflète clairement la transition saisonnière, passant du froid hivernal à la chaleur estivale, avant de redescendre en automne.

-   **Hétérogénéité Statistique :** Certains mois montrent une distribution plus large, traduisant une variabilité interannuelle importante (par exemple, des étés exceptionnellement chauds ou des printemps plus imprévisibles).

-   **Stabilité Relative de Certains Mois :** À l’inverse, des mois affichent des distributions plus resserrées, indiquant une relative stabilité des températures au fil des années.
...
```{r Calcul des anomalies saissonières}
#| echo: false
#| results: "hide"
...
# Associer les noms des rasters avec Année et Mois
names(raster_temp_mean) <- paste0(rep(2004:2023, each = 12), "_", 
                                  rep(c("Jan", "Fév", "Mar", "Avr", "Mai", "Juin", 
                                        "Juil", "Août", "Sept", "Oct", "Nov", "Déc"), times = 20))

# Créer une table pour relier les mois aux saisons climatiques
mois_saison <- data.frame(
  Mois = c("Jan", "Fév", "Mar", "Avr", "Mai", "Juin", 
           "Juil", "Août", "Sept", "Oct", "Nov", "Déc"),
  Saison = c("Hiver", "Hiver", "Printemps", "Printemps", "Printemps", 
             "Été", "Été", "Été", "Automne", "Automne", "Automne", "Hiver")
)

# Transformer le raster stack en data.frame
cat("Transformation du raster en format data.frame...\n")
anomalies <- as.data.frame(raster_temp_mean, xy = TRUE, na.rm = TRUE)

# Renommer les colonnes avec Année et Mois
...
cat("Restructuration des données en format long...\n")
anomalies_long <- anomalies %>%
  ...
  left_join(mois_saison, by = "Mois") # Associer les saisons climatiques

# Calculer la température moyenne annuelle pour chaque saison
cat("Calcul de la température moyenne annuelle pour chaque saison...\n")
moyenne_saison <- anomalies_long %>%
  group_by(Année, Saison) %>% # Agréger par Année et Saison
  summarize(
    Temp_Moyenne_Annuelle = mean(Température, na.rm = TRUE), # Calcul de la moyenne
    .groups = "drop"
  )

# Sauvegarder le DataFrame corrigé au format RDS
saveRDS(moyenne_saison, file = stringr::str_glue("{PLOT_FOLDER}/moyenne_saison_annuelle.rds"))
cat("Le DataFrame des moyennes saisonnières annuelles a été sauvegardé avec succès.\n")

```


```{r Plottage des anomalies saissonières}
#| echo: false
#| fig-width: 14
#| fig-height: 8
#| out.width: "100%"
...
moyenne_saison <- readRDS(stringr::str_glue("{PLOT_FOLDER}/moyenne_saison_annuelle.rds"))

# Calculer la pente (coefficient de régression linéaire) pour chaque saison
pentes <- moyenne_saison %>%
  group_by(Saison) %>%
  summarize(pente = coef(lm(Temp_Moyenne_Annuelle ~ Année, data = pick(everything())))[2])

# Créer des étiquettes formatées pour la légende (nom de la saison et pente sur deux lignes)
labels_legenda <- pentes %>%
  mutate(label = paste0(Saison, "\n(", round(pente, 3), "°C/an)")) %>%
  pull(label)

# Associer les étiquettes formatées aux saisons
names(labels_legenda) <- pentes$Saison

# Créer le graphique des moyennes saisonnières
p_moyennes <- ggplot(moyenne_saison, aes(x = Année, y = Temp_Moyenne_Annuelle, color = Saison, group = Saison)) +
  geom_line(linewidth = 1.5) +  # Ajouter les lignes pour chaque saison
  geom_point(size = 2) +  # Ajouter les points pour chaque observation
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, linetype = "dashed", linewidth = 0.8) +  # Ajouter une tendance
  scale_color_manual(
    values = c("Hiver" = "#5499c7", "Printemps" = "#229954", "Été" = "#cb4335", "Automne" = "#eb984e"),
    labels = labels_legenda,  # Utiliser les étiquettes personnalisées
    limits = c("Été", "Automne", "Printemps", "Hiver")  # Réorganiser l'ordre des saisons dans la légende
  ) +
  labs(
    title = "Évolution des Moyennes de Température par Saison (2004-2023)",  
    subtitle = "Tendances des moyennes saisonnières au fil des années",      
    x = "Année",                                                             
    y = "Température Moyenne (°C)",                                          
    color = "Saison"                                                         
  ) +
  theme_minimal(base_size = 16) +  # Appliquer un thème minimal
  theme(
    plot.background = element_rect(fill = "#2C2C2C", color = NA),  
    panel.background = element_rect(fill = "#2C2C2C", color = NA), 
    ...
    axis.title.y = element_text(face = "bold", size = 18, color = "white",
                                margin = margin(r = 10)),                              
    axis.title.x = element_text(face = "bold", size = 18, color = "white",
                                margin = margin(t = 10)),                              
    axis.text = element_text(size = 18, color = "white"),                              
...
ggsave("results/Evolution_Moyennes_Saisons.png", bg = "#2C2C2C", plot = p_moyennes, width = 14, height = 8, dpi = 300)
...
- **Hiver (Bleu) :**  
    Avec une pente de **0.12°C/an** l'hiver est la saison ayant la **tendance linéaire la plus marquée**, avec une augmentation significative des températures moyennes au fil du temps. Cela suggère un réchauffement notable pour cette saison.

- **Été (Rouge) :**  
    Avec une pente de **0.094°C/an**, l'été présente également une tendance de réchauffement notable, mais légèrement moins accentué que pour l'hiver. Ce réchauffement peut amplifier les vagues de chaleur et leurs impacts au fil du temps.

- **Automne (Jaune) :**  
    L'automne montre une **tendance modérée**, avec une pente de **0.058°C/an** et un réchauffement progressif mais moins prononcé que pour l'été et l'hiver.

- **Printemps (Bleu clair) :**  
    Le printemps a le **slope le plus faible**, indiquant que cette saison est la **moins impactée** en termes de réchauffement.
...
couleurs_regions <- c(
  "Auvergne-Rhône-Alpes" = "#00FFFF",       # Ciano brilhante
  "Bretagne" = "#FF4500",                   # Laranja queimado
  "Bourgogne-Franche-Comté" = "#FFD700",    # Dourado
  "Centre-Val de Loire" = "#FF0000",        # Verde claro
  "Corse" = "#FF1493",                      # Rosa choque
  "Grand Est" = "#1E90FF",                  # Azul real
  "Hauts-de-France" = "#ADFF2F",            # Verde lima
  "Île-de-France" = "#FF6347",              # Tomate
  "Normandie" = "#8A2BE2",                  # Roxo
  "Nouvelle-Aquitaine" = "#FF00FF",         # Magenta
  "Occitanie" = "#FFFFFF",                  # Verde chartreuse
  "Pays de la Loire" = "#8B4513",           # Marrom sela
  "Provence-Alpes-Côte d'Azur" = "#87CEEB"  # Azul céu
)
...
p <- ggplot(temp_region, aes(x = Temp_Min, y = Temp_Max, color = Région, shape = Décennie)) +
  geom_point(alpha = 0.6, size = 4) +  # Points avec transparence et taille optimisée
...
Bien que ce graphique puisse sembler confus en raison de la superposition des points, certaines tendances intéressantes peuvent être mises en évidence.  

- **Corrélation Positive Générale :**  
  Une **relation linéaire positive** est observée entre les températures maximales et minimales moyennes. Les régions ayant des températures minimales plus élevées présentent également des températures maximales plus élevées.  

- **Évolution des Décennies :**  
  - Les points **triangulaires** (2014-2023) se décalent légèrement vers la **droite** et vers le **haut** du graphique, suggérant une augmentation des températures minimales et maximales moyennes par rapport à la décennie précédente (2004-2013).  
  - Cette tendance est particulièrement marquée en **Corse, Nouvelle-Aquitaine, Occitanie et Bretagne**, où les étés ont été plus chauds (**présence accrue de triangles en haut**) et les hivers caractérisés par des températures minimales plus élevées (**triangles plus nombreux à droite**).  


## **Détails Techniques**  

1. **Extraction des Températures Maximales et Minimales :**  
   Les températures moyennes **maximales** et **minimales** ont été extraites pour chaque année et chaque région à partir des données **TerraClimate**. Cette étape a été réalisée via des opérations sur les rasters climatiques et les shapefiles des régions françaises.  

2. **Agrégation des Moyennes Régionales :**  
   Les valeurs ont été moyennées au niveau régional afin d’assurer une analyse cohérente et de faciliter la comparaison entre les régions.  

3. **Différenciation des Décennies :**  
   Une colonne supplémentaire a été ajoutée aux données pour distinguer les années appartenant à la première décennie (**2004-2013**) et celles de la seconde (**2014-2023**).  

4. **Visualisation des Données :**  
   - Les **couleurs** des points permettent de différencier les régions françaises.  
   - Les **formes** des points (**cercles et triangles**) indiquent la décennie correspondante, facilitant ainsi la comparaison temporelle.
...
1.  **Agrégation Temporelle et Géographique :**
...
2.  **Analyse des Tendances Linéaires :**
